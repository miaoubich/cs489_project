...
class CustomerControllerTests {
	...
	@DisplayName("Register new Customer in the database!")
	public void createCustomerTest() throws Exception {
		...
		this.mockMvc
				.perform(
						post("/video-rental-store/api/v1/customer").header(HttpHeaders.AUTHORIZATION, "Bearer " + token)
	...
	public void getAllCustomersTest() throws Exception {
		...
		this.mockMvc
				.perform(get("/video-rental-store/api/v1/customer").header("Authorization", "Bearer " + token)
	...
	@Test
	@WithMockUser
	public void getAllCustomersWithoutSecurityTokenTest() throws Exception {
		List<Customer> customers = buildCustomers();
	...
	public CustomerRequest buildCustomerRequest() {
		return CustomerRequest.builder().id(customerId).firstName("Ali").lastName("Bouzar").email("ali@email.net")
				.rentals(Collections.emptyList()).rentals(rentalRequests()).build();
	}

	public CustomerResponse buildCustomerResponse() {
		return CustomerResponse.builder().id(customerId).firstName("Ali").lastName("Bouzar").email("ali@email.net")
				.rentals(rentalResponses()).build();
	}

	public Customer buildCustomer() {
		return Customer.builder().id(customerId).firstName("Ali").lastName("Bouzar").email("ali@email.net")
	...
	public List<Customer> buildCustomers() {
		...
		Customer customer1 = Customer.builder().id(customerId).firstName("Ali").lastName("Bouzar")
				.email("ali@email.net").rentals(rentals()).build();
		Customer customer2 = Customer.builder().id(customerId1).firstName("Sam").lastName("Ben").email("sam@email.net")
				.rentals(rentals()).build();
