...
public class CustomerServiceImpl implements CustomerService {
	...
	public CustomerResponse createCustomer(CustomerRequest customerRequest) {
		Optional<Customer> customerExist = Optional
				.ofNullable(customerRepository.getCustomerByEmail(customerRequest.getEmail()));
		CustomerResponse customerResponse = null;

		if (!customerExist.isPresent()) {
			log.info("Mapping RentalRequest from CustomerRequest Object into Rental object ...");
			List<Rental> rentals = customerRequest.getRentals().stream()
					.map(rentalRequest -> new Rental(rentalRequest.getRentalDate(), rentalRequest.getReturnDate()))
					.toList();
			...
			List<RentalResponse> rentalResponses = savedRentals.stream()
					.map(r -> new RentalResponse(r.getId(), r.getRentalDate(), r.getReturnDate())).toList();
			...
			throw new CustomerServiceCustomException(
					"A Customer with Email: - " + customerRequest.getEmail() + " - already exist!", "CUSTOMER_EXIST",
					HttpStatus.FOUND);
		}
		log.info("Customer " + customerResponse.getFirstName() + " " + customerResponse.getLastName()
				+ " already exist!");
	...
	public CustomerResponse updateCustomer(Long customerId, CustomerRequest customerRequest) {
		...
		if (customerRequest.getRentals() != null && existCustomer.getRentals() != null) {
			existCustomer.getRentals().stream().map(existRental -> {
				RentalRequest rentalRequest = customerRequest.getRentals().stream()
						.filter(re -> re.getId() == existRental.getId()).findFirst().get();
	...
	private CustomerResponse mapper(Customer customer) {
		...
		List<RentalResponse> rentals = customer.getRentals().stream().map(rental -> {
